...
from modelizer.generators.implementations import Re2_Subject, Re2_Generator
...
# USAGE: python scripts/subjects/generate_re2.py -c 10 -d /mount/ -f coverage -min 1 -max 24
...
if __name__ == "__main__":
    ...
    path = f"{args.data_dir}re2-{args.count}-{args.fuzzer[0]}-{args.min}_{args.max}.pkl"

    subj = Re2_Subject()
    if args.fuzzer == "coverage":
        gen = Re2_Generator(subject=subj, seed=SEED, fuzzer_factory=GrammarCoverageFuzzer, min_nonterminals=args.min, max_nonterminals=args.max)
    elif args.fuzzer == "kpath":
        gen = Re2_Generator(subject=subj, seed=SEED, fuzzer_factory=KPathGrammarFuzzer, min_nonterminals=args.min, max_nonterminals=args.max)
