...
from modelizer.generators.implementations import BC_Subject, BC_Generator
...
# USAGE: python scripts/subjects/generate_bc.py -c 10 -d /mount/ -f coverage -min 1 -max 24
...
if __name__ == "__main__":
    ...
    path = f"{args.data_dir}bc-{args.count}-{args.fuzzer[0]}-{args.min}_{args.max}.pkl"

    subj = BC_Subject()
    if args.fuzzer == "coverage":
        gen = BC_Generator(subject=subj, seed=SEED, fuzzer_factory=GrammarCoverageFuzzer, min_nonterminals=args.min, max_nonterminals=args.max)
    elif args.fuzzer == "kpath":
        gen = BC_Generator(subject=subj, seed=SEED, fuzzer_factory=KPathGrammarFuzzer, min_nonterminals=args.min, max_nonterminals=args.max)
